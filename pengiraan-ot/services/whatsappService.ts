 ...
 *  5. Masukkan token dalam FONNTE_TOKEN di bawah
 *  6. Deploy semula
 *
 * Fonnte pricing: ~RM10/bulan untuk 1000 mesej
 *
 * ── FALLBACK: wa.me LINK ────────────────────────────────────────────────────
 * Jika Fonnte belum dikonfigurasi, sistem akan buka wa.me link dengan mesej
 * siap-tulis (user perlu klik Hantar).
 */

// ── KONFIGURASI UTAMA ─────────────────────────────────────────────────────
// Nombor ini akan MENGHANTAR semua notifikasi WA kepada staff/HOD/Admin
export const SENDER_PHONE = '60129444295';  // +60129444295

// Token Fonnte — dapatkan dari dashboard fonnte.com selepas setup
// Biarkan kosong ('') untuk guna wa.me link (manual)
export const FONNTE_TOKEN = 'VofHwA1HhdzAy4X2aa5s';
...
async function sendFonnte(targetPhone: string, message: string): Promise<boolean> {
  ...
  try {
    ...
    console.log(`[WhatsApp/Fonnte] ✓ Dihantar ke ${targetPhone}`);
    return true;
  } catch (err) {
...
async function sendToRecipient(phone: string, message: string): Promise<boolean> {
  ...
  // Cuba auto-send via Fonnte (dari +60129444295)
  const sent = await sendFonnte(phone, message);
  return sent;
}
...
export async function waNotifyHOD(params: {
  hodPhone: string;
  hodWaKey?: string;            // legacy, tidak digunakan
  hodName: string;
  ...
  const msg =
    ...
    `🔗 https://ksbsb-leave-trcker.web.app`;
...
export async function waNotifyAdmin(params: {
  adminPhone: string;
  adminWaKey?: string;          // legacy, tidak digunakan
  adminName: string;
  applicantName: string;
  leaveType: string;
  duration: number;
...
export async function waNotifyApplicant(params: {
  ...
  applicantWaKey?: string;      // legacy, tidak digunakan
  applicantName: string;
  leaveType: string;
  duration: number;
  startDate: string;
  ...
  const msg =
    ...
    (isApproved && params.remainingBalance !== undefined ? `📝 Baki Cuti Terkini: *${params.remainingBalance} hari*\n` : '') +
