...
### Cuti Ganti = jenis baru `CG`

- **Kod:** `CG`. **Nama:** `Cuti Ganti`. **Warna:** `#14b8a6` (teal).
- **Doktor sahaja.** Ditapis dalam borang mohon dengan `user.category === 'Doctor'`,
  ...
  (`src/main.js:10326`).
- **Tiada kuota tetap.** `entitlement: 0`, dan `CG` **tidak** ditambah ke
...
- **Tiada medan Management Hub.** Kerana tiada kuota, HR tiada apa untuk ditetapkan.
  `CG` tidak muncul dalam tatasusunan `leaveTypes` di `src/main.js:5121` dan tiada
  input `ent-CG`.
- **Hari bekerja**, bukan hari kalendar — `CG` tidak ditambah ke
  `CALENDAR_DAY_LEAVE_TYPES`.
- **Laluan kelulusan biasa.** Tiada pengecualian `shouldSkipP1`, tiada muat naik bukti
  diwajibkan, tiada pengecualian notis (`isNoticeExempt` kekal seperti sedia ada).
- **Nota pada borang:** "Cuti ganti selepas menghadiri mesyuarat doktor."

## Seni bina

Dua peta paparan diperkenalkan di `src/main.js`, bersebelahan `LEAVE_TYPE_NAMES`
(`src/main.js:617`):

```js
// Nama penuh — diterbitkan daripada leaveCategories (sedia ada, tidak berubah bentuk).
...
const LEAVE_TYPE_SHORT = Object.freeze({ EL_EMG: 'EMG', ML_PL: 'PL' });
function leaveTypeShort(code) { return LEAVE_TYPE_SHORT[code] || code; }
```
...
| --------------------- | ------------------------------------------------------------------------------- |
| `leaveCategories:605` | Nama baru untuk `EL`, `EL_EMG`, `ML_PL`; tambah entri `CG`                       |
| selepas `:619`        | Tambah `LEAVE_TYPE_SHORT` + `leaveTypeShort()`                                   |
| `:2148`               | Borang cuti cetak — tambah kotak `[ ] CUTI GANTI`                                |
| `:2873`               | `LEAVE_TYPE_COLOR` — tambah `CG:'#14b8a6'`                                       |
| `:2874`               | `ALL_LEAVE_TYPES` — tambah `'CG'`                                                |
| `:2877`               | Buang `leaveReportLabel()`; pemanggil beralih ke `leaveTypeName()`               |
| `:3256`, `:8195`      | `typeColors` / `typeColorMap` — tambah `CG`                                      |
...
| `:6039`               | `filteredCategories` — `CG` hanya untuk `user.category === 'Doctor'`             |
| `:6062`, `:6073`      | `leaveIcons` + `leaveShort` — tambah `CG`                                        |
...
| glosari `:521`        | Help-bot — tambah `CG`; betulkan nama `EL`/`EL_EMG`                              |
...
`selectedLeaveType === 'CG'` tetapi pengguna bukan doktor, kembali ke `'AL'`. Ini
...
Tiada perubahan. Permohonan `CG` mengalir sama seperti `EL_EMG`: `record.type = 'CG'`,
...
`getLeaveStats(staff, 'CG')` mengembalikan `ent: 0` (tiada `ent_CG` tersimpan, dan
...
- **Bukan doktor memohon `CG`** — tidak boleh berlaku melalui antara muka kerana
...
- `tests/leaveTypeLabels.test.mjs` (baru) — mengesahkan `id` dalam `leaveCategories`
  unik, setiap kod pendek unik selepas `leaveTypeShort()` dipakai (menangkap
  pertindihan `EL`/`EMG`), dan `CG` bukan ahli `FORMULA_B_TYPES`.
- `tests/formulaBTypes.test.mjs` sedia ada mesti kekal lulus tanpa diubah — `CG` tidak
  menyentuh Formula B.
- Semakan manual: mohon satu Cuti Ganti sebagai doktor, luluskan, dan sahkan ia muncul
  dalam laporan Semua Cuti serta jadual Analisa dengan lajur `CG`.
