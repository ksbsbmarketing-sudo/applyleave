...
**Zone scoping is not optional here — but the state and branch tabs must not narrow the pairing itself.** Leave records snapshot `branch: user.branch` at apply time, so a staff transfer splits one person's history across branch values; the state and branch tabs are navigation over that history, not a security boundary. Pairing the overlap map from the tab-narrowed `_mlRecords` would make a duplicate split across branches invisible the moment a branch tab is selected — both records unmarked, count and all. So the overlap Map is built from a second, zone-only pass (`_mlZoneRecords`): `userScope` is kept, `state` and `branch` are relaxed to `SCOPE_ALL`. The viewer's zone — the actual security boundary — is still enforced there; a Pahang HR still cannot see a Terengganu record. The *rendered rows* still come from `_mlRecords`, narrowed by the current state/branch tab as before. One consequence of the two-list split: the count `N` (rows in the current tab that are flagged) and a row's tooltip (naming every record it overlaps, drawn from the zone-wide map) can have different denominators — a tooltip may name a record that lives outside the branch tab you're currently looking at.
...
One blind spot remains, honestly stated: a pair split across two **zones** (a staff member transferred from a Pahang branch to a Terengganu one, say) is surfaced to neither HR's Master Logs, because each HR's `_mlZoneRecords` only ever contains their own zone's half of the pair. That is the zone boundary correctly winning — pairing across zones to surface it would leak one zone's record details into the other's view, which is worse. Surfacing is degraded, but **prevention still works cross-zone**: the guard in `finalizeLeave` calls `findApprovedOverlaps` over the full in-memory `leaveRecords`, not a zone-scoped list, so approving the second half of a cross-zone pair is still blocked even though neither Master Logs view ever showed the pair as flagged.
